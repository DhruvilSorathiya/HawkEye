...
class HawkEyeDb:
    ...
    createSchemaQueries = [
        ...
        "CREATE INDEX idx_admins_username ON tbl_admins (username)",
    ...
    def __init__(self, opts: dict, init: bool = False) -> None:
        ...
        # persist full config for later use (e.g., per-call sqlite connections)
        self.config = opts
        ...
        with self.dbhLock:
            ...
            # For users with pre-login system databases, add the authentication
            # tables + indexes if they don't exist.
            try:
                self.dbh.execute("SELECT COUNT(*) FROM tbl_admins")
            except sqlite3.Error:
                # Authentication tables don't exist, create them
                print("Creating authentication tables...")
                try:
                    for query in self.createSchemaQueries:
                        if any(table in query for table in ["tbl_users", "tbl_admins", "tbl_user_sessions", "tbl_user_activity"]):
                            self.dbh.execute(query)
                    self.conn.commit()
                    print("Authentication tables created successfully")
                except sqlite3.Error as e:
                    print(f"Error creating authentication tables: {e}")
                    raise IOError("Failed to create authentication tables. Please delete your database and restart.") from None
    ...
    def createDefaultAdmin(self) -> bool:
        ...
        with self.dbhLock:
            try:
                ...
                self.dbh.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='tbl_admins'")
                if not self.dbh.fetchone():
                    # Table doesn't exist yet, skip creating admin
                    return False
                
                # Check if admin already exists
                self.dbh.execute("SELECT COUNT(*) FROM tbl_admins WHERE username = ?", ("admin",))
                if self.dbh.fetchone()[0] > 0:
                    return True
                
                # Create default admin
                password_hash = hashlib.sha256("admin".encode()).hexdigest()
                qry = "INSERT INTO tbl_admins (username, password_hash, email, created_at, is_active) VALUES (?, ?, ?, ?, ?)"
                self.dbh.execute(qry, ("admin", password_hash, "admin@hawkeye.local", int(time.time()), 1))
                self.conn.commit()
                return True
            except sqlite3.Error as e:
                # If there's an error, it might be because the table doesn't exist yet
                # This is okay, the table will be created later
                return False
    ...
    def authenticateUser(self, username: str, password: str) -> dict:
        ...
        with self.dbhLock:
            try:
                self.dbh.execute("SELECT id, username, email, mobile, last_login FROM tbl_users WHERE username = ? AND password_hash = ? AND is_active = 1", 
                               (username, password_hash))
                result = self.dbh.fetchone()
                if result:
                    # Update last login
                    self.dbh.execute("UPDATE tbl_users SET last_login = ? WHERE id = ?", (int(time.time()), result[0]))
                    self.conn.commit()
                    return {
                        'id': result[0],
                        'username': result[1],
                ...
                raise IOError("SQL error encountered when authenticating user") from e

    def authenticateAdmin(self, username: str, password: str) -> dict:
        """Authenticate an admin in a thread-safe way using a new connection per request."""
        import sqlite3
        import hashlib
        import time
        ...
        try:
            ...
            conn = sqlite3.connect(self.config['__database'])
            cursor = conn.cursor()

            # Fetch admin
            cursor.execute(
                "SELECT id, username, email, last_login FROM tbl_admins "
            ...
            if result:
                # Update last login
                cursor.execute("UPDATE tbl_admins SET last_login = ? WHERE id = ?", (int(time.time()), result[0]))
                conn.commit()
                return {
                    'id': result[0],
                    'username': result[1],
            ...
            raise IOError("SQL error encountered when authenticating admin") from e
        finally:
            conn.close()

    def createUser(self, username: str, password: str, email: str, mobile: str, created_by_admin_id: int) -> bool:
        """Create a new user.
        ...
        Args:
            ...
            created_by_admin_id (int): admin who created this user
        ...
        with self.dbhLock:
            try:
                qry = "INSERT INTO tbl_users (username, password_hash, email, mobile, created_at, created_by_admin_id) VALUES (?, ?, ?, ?, ?, ?)"
    ...
    def createAdmin(self, username: str, password: str, email: str, created_by_admin_id: int) -> bool:
        ...
        self._ensure_admin_mobile_column()
        ...
        with self.dbhLock:
            try:
                ...
                raise IOError("SQL error encountered when creating admin") from e
    ...
    def getAllUsers(self) -> list:
        ...
        with self.dbhLock:
            try:
                self.dbh.execute("SELECT id, username, email, mobile, created_at, last_login, is_active FROM tbl_users ORDER BY created_at DESC")
                return self.dbh.fetchall()
            except sqlite3.Error as e:
    ...
    def getAdminCount(self) -> int:
        ...
        with self.dbhLock:
            try:
                ...
                # If table missing, treat as zero to allow first-time setup
                return 0

    def _ensure_admin_mobile_column(self) -> None:
        """Add mobile column to tbl_admins if missing."""
        with self.dbhLock:
            try:
                self.dbh.execute("PRAGMA table_info(tbl_admins)")
                cols = [r[1] for r in self.dbh.fetchall()]
                if 'mobile' not in cols:
                    self.dbh.execute("ALTER TABLE tbl_admins ADD COLUMN mobile VARCHAR")
                    self.conn.commit()
            except sqlite3.Error:
                pass
    ...
    def createSession(self, user_id: int = None, admin_id: int = None, ip_address: str = None, user_agent: str = None) -> str:
        ...
        with self.dbhLock:
            try:
                qry = "INSERT INTO tbl_user_sessions (id, user_id, admin_id, created_at, expires_at, ip_address, user_agent) VALUES (?, ?, ?, ?, ?, ?, ?)"
                self.dbh.execute(qry, (session_id, user_id, admin_id, int(time.time()), expires_at, ip_address, user_agent))
                self.conn.commit()
                return session_id
            except sqlite3.Error as e:
                raise IOError("SQL error encountered when creating session") from e

    def getSession(self, session_id: str) -> dict:
        ...
        with self.dbhLock:
            try:
                self.dbh.execute("""
                    SELECT id, user_id, admin_id, created_at, expires_at, ip_address, user_agent 
                ...
                raise IOError("SQL error encountered when fetching session") from e
    ...
    def deleteSession(self, session_id: str) -> bool:
        ...
        with self.dbhLock:
            try:
                self.dbh.execute("DELETE FROM tbl_user_sessions WHERE id = ?", (session_id,))
                self.conn.commit()
                return True
            except sqlite3.Error as e:
